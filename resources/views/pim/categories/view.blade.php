<x-layout.default>
    <form action="{{ route('pim.categories.update', ['id' => $category->id]) }}" method="POST" >
        @csrf
            ...
            <div class="px-0 flex-1 py-0 ltr:xl:mr-1 rtl:xl:ml-1">
                <div class="panel mb-5 flex items-center gap-3">
                    ...
                    <input disabled
                        ...
                        value="http://127.0.0.1:8000/pim/categories/{{ basename(old('slug', $category->slug)) }}"
                        readonly
                    />
                    ...
                    <input disabled type="number" name="sorting" id="sorting" class="form-input w-24" placeholder='0' value='{{ old('sorting', $category->sorting) }}'>
                </div>
                <div class="mb-5" x-data="{
                    tab: 'English',
                    name: '{{ old('name', $category->name) }}',
                    h1: '{{ old('h1', $category->h1_en) }}',
                    category_description: '{{ old('category_description', $category->description) }}',
                    nameAr: '{{ old('nameAr', $category->name_arabic) }}',
                    h1Ar: '{{ old('h1Ar', $category->h1_arabic) }}',
                    category_descriptionAr: '{{ old('category_descriptionAr', $category->description_arabic) }}'
                }">
                    <div>
                ...
                <div class="mb-5" x-data="{
                    tab: 'MetaData - En',
                    metatitle: '{{ old('metatitle', $category->meta_title) }}',
                    metatags: '{{ old('metatags', $category->meta_tags) }}',
                    meta_canonical: '{{ old('meta_canonical', $category->meta_canonical) }}',
                    metadescription: '{{ old('metadescription', $category->meta_description) }}',
                    metatitleAr: '{{ old('metatitleAr', $category->meta_title_arabic) }}',
                    metatagsAr: '{{ old('metatagsAr', $category->meta_tags_arabic) }}',
                    meta_canonicalAr: '{{ old('meta_canonicalAr', $category->meta_canonical_arabic) }}',
                    metadescriptionAr: '{{ old('metadescriptionAr', $category->meta_description_arabic) }}',
                }">
                    <div>
                        <ul class="flex flex-wrap mt-3">
                            <li>
                ...
                <div class="mb-5" x-data="{
                    tab: 'Content - En',
                    contentTitleEn: '{{ old('contenttitle', $category->content_title) }}',
                    contentDescEn: '{{ old('contentdescription', $category->content_description) }}',
                    contentTitleAr: '{{ old('contenttitleAr', $category->content_title_arabic) }}',
                    contentDescAr: '{{ old('contentdescriptionAr', $category->content_description_arabic) }}'
                }">
                    <div>
                        <ul class="flex flex-wrap mt-3">
                            <li>
            ...
            <div class="xl:w-96 w-full xl:mt-0 mt-6">
                <div class="panel mb-3">
                    <div class="flex xl:grid-cols-1 lg:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-2">
                        <a
                            ...
                            href="{{ route('pim.categories.listing') }}"
                        >
                            <svg
                    ...
                    <div class="grid grid-cols-2 gap-3 mt-3">
                        ...
                        <label class="mt-3 b-0">
                            <input disabled
                                ...
                                class="form-checkbox"
                                value="1"
                                {{ old('not_for_export', $category->not_for_export) == '1' ? 'checked' : '' }}
                            />
                        ...
                        <label class="mt-3 b-0">
                            <input disabled
                                ...
                                class="form-checkbox"
                                value="1"
                                {{ old('show_in_menu', $category->show_in_menu) == '1' ? 'checked' : '' }}
                            />
                        ...
                        <label class="mt-3 b-0">
                            <input disabled
                                ...
                                class="form-checkbox"
                                value="1"
                                {{ old('show_on_arabyads', $category->show_on_arabyads) == '1' ? 'checked' : '' }}
                            />
                ...
                <div class="mb-3 panel">
                    <div class="grid grid-cols-2 gap-3">
                        <label class="mb-0">
                            <input disabled
                                ...
                                class="form-checkbox"
                                value="1"
                                {{ old('no_follow', $category->nofollow_analytics) == '1' ? 'checked' : '' }}
                            />
                        ...
                        <label class="mb-0">
                            <input disabled
                                ...
                                class="form-checkbox"
                                value="1"
                                {{ old('no_index', $category->noindex_analytics) == '1' ? 'checked' : '' }}
                            />
                    ...
                    <div class='mt-5'>
                        ...
                        <input disabled type="text" name="redirection_link" id="redirection_link" class="form-input" value='{{ old('redirection_link', $category->redirection_link) }}' placeholder='Enter Redirection Link'>
                        @error('redirection_link')
                            <small style="color: #b91c1c;" class="mt-1">{{ $message }}</small>
                        @enderror
                    </div>
                </div>
                <div class="mb-3 panel space-y-3">
                    <div x-data="{ selectedType: '{{ old('selltype', $category->sell_type) ?? '' }}' }">
                        <label for="">Sell Type</label>
                        <select disabled class="form-select" name="selltype" id="selltype" x-model="selectedType">
                            <option value="" {{ old('selltype', $category->sell_type ?? '0') == '0' ? 'selected' : '' }}>Select</option>
                            <option value="1" {{ old('selltype', $category->sell_type ?? '0') == '1' ? 'selected' : '' }}>Percentage</option>
                            <option value="2" {{ old('selltype', $category->sell_type ?? '0') == '2' ? 'selected' : '' }}>Save</option>
                        </select>
                    </div>
                    <div>
                        <label for="">Value</label>
                        <input disabled type="text" name='value' id='value' class='form-input' value='{{ old('value', $category->value) }}'>
                    </div>
                </div>
                <div class="mb-3 panel space-y-3">
                    <div>
                        ...
                        <select disabled id="selectCat" class="form-select" name="selectCat">
                            <option value="" disabled {{ old('selectCat', $category->parent_category) === null ? 'selected' : '' }}>Select parent Category</option>
                            @if(is_iterable($cats))
                                @foreach($cats as $cat)
                                    <option value="{{ $cat->id }}" {{ old('selectCat', $category->parent_category) == $cat->id ? 'selected' : '' }}>
                                        {{ $cat->name }}
                                    </option>
                                @endforeach
                            @endif
                    ...
                    <div>
                        ...
                        <select disabled name="selectSubtag[]" id="selectSubtag" class="w-full form-multiselect" multiple>
                            <option value="" disabled>Select</option>
                            @foreach($subTagsOptions as $subTag)
                                <option value="{{ $subTag->id }}"
                                    {{ in_array($subTag->id, old('selectSubtag', $selectedFilters)) ? 'selected' : '' }}>
                                    {{ $subTag->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3 panel space-y-3">
                    <div>
                        ...
                        <textarea disabled name="upload_icon" id="upload_icon" class="form-input h-20" placeholder='Icon...'>{{ old('upload_icon', $category->icon) }}</textarea>
                    </div>
                    <div>
                        <label for="brand_link">Brand Link</label>
                        <input disabled type="text" name="brand_link" id="brand_link" class="form-input" placeholder='Link' value="{{ old('brand_link', $category->brand_link) }}">
                    </div>
                    <div>
                        ...
                        <input disabled type="text" name="image_link" id="image_link" class="form-input" placeholder='Link' value="{{ old('image_link', $category->image_link) }}">
                    </div>
                </div>
                <div class="panel mb-3">
                    ...
                    <div class="relative rounded-md mt-3 overflow-hidden w-full max-w-sm" id="mobilePreview">
                        @php
                            $image_mobile = old('image_mobile', $category->mobile_image);
                        @endphp
                        @if (!empty($image_mobile))
                            <div class="relative inline-block">
                                <button disabled type="button" class="text-danger text-xl font-bold z-10 absolute top-0 left-1"
                ...
                <div class="panel mb-3">
                    ...
                    <div class="relative rounded-md mt-3 overflow-hidden w-full max-w-sm" id="websitePreview">
                        @php $image_website = old('image_website', $category->website_image) ?? $image_website ?? ''; @endphp
    ...
    <script>
        const baseSlug = "http://127.0.0.1:8000/pim/categories/";
