<x-layout.default>
    <form action="{{ route('pim.categories.store') }}" method="POST" >
        @csrf
            ...
            <div class="px-0 flex-1 py-0 ltr:xl:mr-1 rtl:xl:ml-1">
                <div class="panel mb-5 flex items-center gap-3">
                    ...
                    <input
                        ...
                        value="http://127.0.0.1:8000/pim/categories/{{ basename(old('slug')) }}"
                        readonly
                    />
                    ...
                    <input type="number" name="sorting" id="sorting" class="form-input w-24" placeholder='0' value='{{ old('sorting') }}'>
                </div>
                <div class="mb-5" x-data="{
                    ...
                    nameAr: '{{ old('nameAr') }}',
                    h1Ar: '{{ old('h1Ar') }}',
                    category_descriptionAr: '{{ old('category_descriptionAr') }}'
                }">
                    <div>
                ...
                <div class="mb-5" x-data="{
                    ...
                    meta_canonical: '{{ old('meta_canonical') }}',
                    metadescription: '{{ old('metadescription') }}',
                    metatitleAr: '{{ old('metatitleAr') }}',
                    metatagsAr: '{{ old('metatagsAr') }}',
                    meta_canonicalAr: '{{ old('meta_canonicalAr') }}',
                    metadescriptionAr: '{{ old('metadescriptionAr') }}',
                }">
                    <div>
                        <ul class="flex flex-wrap mt-3">
                            <li>
                ...
                <div class="mb-5" x-data="{
                    tab: 'Content - En',
                    contentTitleEn: '{{ old('contenttitle') }}',
                    contentDescEn: '{{ old('contentdescription') }}',
                    contentTitleAr: '{{ old('contenttitleAr') }}',
                    contentDescAr: '{{ old('contentdescriptionAr') }}'
                }">
                    <div>
                        <ul class="flex flex-wrap mt-3">
                            <li>
            ...
            <div class="xl:w-96 w-full xl:mt-0 mt-6">
                    ...
                    <div class="grid grid-cols-2 gap-3 mt-3">
                        ...
                        <label class="mt-3 b-0">
                            <input
                                ...
                                class="form-checkbox"
                                value="1"
                                {{ old('show_on_arabyads') == '1' ? 'checked' : '' }}
                            />
                ...
                <div class="mb-3 panel">
                    ...
                    <div class='mt-5'>
                        ...
                        <input type="text" name="redirection_link" id="redirection_link" class="form-input" value='{{ old('redirection_link') }}' placeholder='Enter Redirection Link'>
                ...
                <div class="mb-3 panel space-y-3">
                    <div x-data="{ selectedType: '{{ old('selltype') ?? '' }}' }">
                        <label for="">Sell Type</label>
                        <select class='form-select' name='selltype' id='selltype' placeholder='Select'  x-model="selectedType">
                            <option value="" {{ old('selltype') == '' ? 'selected' : '' }}>Select</option>
                            <option value="1" {{ old('selltype', '1') == '1' ? 'selected' : '' }}>Percentage</option>
                            <option value="2" {{ old('selltype') == '2' ? 'selected' : '' }}>Save</option>
                        </select>
                    </div>
                    <div>
                        <label for="">Value</label>
                        <input type="text" name='value' id='value' class='form-input' value='{{ old('value') }}'>
                    </div>
                </div>
                <div class="mb-3 panel space-y-3">
                    <div>
                        ...
                        <select id="selectCat" class="form-select" name="selectCat">
                            <option value="" disabled {{ old('selectCat') === null ? 'selected' : '' }}>Select parent Category</option>
                            @if(is_iterable($cats))
                                @foreach($cats as $cat)
                                    <option value="{{ $cat->id }}" {{ old('selectCat') == $cat->id ? 'selected' : '' }}>
                                        {{ $cat->name }}
                                    </option>
                                @endforeach
                            @endif
                    ...
                    <div>
                        ...
                        <select name="selectSubtag[]" id="selectSubtag" class="w-full form-multiselect" multiple>
                            ...
                            @if(is_iterable($subTagsOptions))
                                @foreach($subTagsOptions as $subTag)
                                    <option value="{{ $subTag->id }}" {{ (collect(old('selectSubtag'))->contains($subTag->id)) ? 'selected' : '' }}>
                                        {{ $subTag->name }}
                                    </option>
                                @endforeach
                            @endif
                ...
                <div class="mb-3 panel space-y-3">
                    <div>
                        ...
                        <textarea name="upload_icon" id="upload_icon" class="form-input h-20" placeholder='Icon...'>{{ old('upload_icon') }}</textarea>
                    </div>
                    <div>
                        <label for="brand_link">Brand Link</label>
                        <input type="text" name="brand_link" id="brand_link" class="form-input" placeholder='Link' value="{{ old('brand_link') }}">
                    </div>
                    <div>
                        ...
                        <input type="text" name="image_link" id="image_link" class="form-input" placeholder='Link' value="{{ old('image_link') }}">
                    </div>
                </div>
                <div class="panel mb-3">
                ...
                <div class="panel mb-3">
                    ...
                    <div class="relative rounded-md mt-3 grid grid-cols-3 gap-3">
                        @php
                            $imageWebsite = old('image_website', $product->image_website ?? '');
                            $galleryImages = $imageWebsite ? explode(',', $imageWebsite) : [];
                        @endphp
                        @foreach ($galleryImages as $url)
                            @php $cleanUrl = e($url); @endphp
                            <div class="relative inline-block panel">
                                <button type="button"
                                        class="text-danger text-xl font-bold z-10 absolute top-0 left-1"
                    ...
                    <input type="hidden" id="image_website_hidden" name="image_website" value="{{ old('image_website', $imageWebsite) }}">
                </div>
            </div>
        </div>
    </form>
