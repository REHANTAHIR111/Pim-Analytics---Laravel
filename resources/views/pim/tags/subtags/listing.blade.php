@php
    $subtag = session('subtag_edit_data');
    $subtag_view = session('subtag_view_data');
@endphp
<x-layout.default>
        ...
        <div class="panel mt-6">
            <div class="flex gap-3 absolute">
                @if($permission && $permission->delete)
                    <form id="bulk-delete-form" method="POST" action="{{ route('pim.tags.subtags.bulk-delete') }}">
                        @csrf
                ...
                @if($permission && $permission->create)
                    <div x-data="modal" x-init="@if(session('open_modal') === 'create_subtag') setTimeout(() => open = true, 500); @endif">
                            ...
                            <div class="flex items-start justify-center min-h-screen px-4"
                                ...
                                <div class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-4xl my-20 animate__animated"
                                    ...
                                    <div
                                        ...
                                        <h5 class="font-bold text-lg text-gray-300">Add New SubTags</h5>
                                    ...
                                    <div class="p-5">
                                        <form action={{ route('pim.tags.subtags.store') }} method="POST">
                                            @csrf
                                            <input type="hidden" name="tag_id" value="{{ request()->route('id') }}">
                                            <div>
                                                <div class="grid grid-cols-2 w-full gap-3">
                                                    <div>
                                                        <label>SubTag Name - En</label>
                                                        <input type="text" class="form-input" id='subTag' name="subTagNameEn" value="{{ old('subTagNameEn') }}" placeholder="SubTag Name - En">
                                                        @error('subTagNameEn')
                                                            <small style="color: #b91c1c;" class="mt-1">{{ $message }}</small>
                                                        @enderror
                                                    </div>
                                                    <div>
                                                        <label>SubTag Name - Ar</label>
                                                        <input type="text" class="form-input" id='subTag' name="subTagNameAr" value="{{ old('subTagNameAr') }}" placeholder="SubTag Name - Ar">
                                                        @error('subTagNameAr')
                                                            <small style="color: #b91c1c;" class="mt-1">{{ $message }}</small>
                                                        @enderror
                                                    </div>
                ...
                @endif
                <div x-data="modal" x-init="@if(session('open_modal') === 'edit_subtag') setTimeout(() => open = true, 500); @endif">
                    <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                        :class="open && '!block'">
                        <div class="flex items-start justify-center min-h-screen px-4"
                            @click.self="open = false">
                            <div class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-4xl my-20 animate__animated"
                                ...
                                <div
                                    class="flex bg-gradient-to-b from-[#16222a] to-[#3a6073] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                                    <h5 class="font-bold text-lg text-gray-300">Edit SubTags</h5>
                                    <button type="button" class="text-white-dark hover:text-dark"
                                        @click="toggle; window.location.reload();">
                                ...
                                <div class="p-5">
                                    <form action="{{ route('pim.tags.subtags.update', ['id' => $subtag ? $subtag->id : 0]) }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $subtag ? $subtag->id : '' }}">
                                        <div>
                                            <div class="grid grid-cols-2 w-full gap-3">
                                                <div>
                                                    <label>SubTag Name - En</label>
                                                    <input type="text" class="form-input" name="subTagNameEn" value="{{ old('subTagNameEn', $subtag ? $subtag->name : '') }}" placeholder="SubTag Name - En">
                                                    @error('subTagNameEn')
                                                        <small style="color: #b91c1c;" class="mt-1">{{ $message }}</small>
                                                    @enderror
                                                </div>
                                                <div>
                                                    <label>SubTag Name - Ar</label>
                                                    <input type="text" class="form-input" name="subTagNameAr" value="{{ old('subTagNameAr', $subtag ? $subtag->name_ar : '') }}" placeholder="SubTag Name - Ar">
                                                    @error('subTagNameAr')
                                                        <small style="color: #b91c1c;" class="mt-1">{{ $message }}</small>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="w-full mt-3">
                                                <label>Sorting</label>
                                                <input type="number" class="form-input" name="sorting" value="{{ old('sorting', $subtag ? $subtag->sorting : '') }}" placeholder="0">
                                            </div>
                                            <div class="w-full mt-3">
                                                <label>Upload Icon</label>
                                                <input type="text" class="form-input" name="uploadIcon" value="{{ old('uploadIcon', $subtag ? $subtag->icon : '') }}">
                                            </div>
                                            <div class="w-full mt-3">
                                                <label>Image Link App</label>
                                                <input type="text" class="form-input" name="imageLink" value="{{ old('imageLink', $subtag ? $subtag->image_link_app : '') }}">
                                            </div>
                                        </div>
                                        <div class="flex justify-between items-center mt-8">
                                            <label class="mb-0">
                                                <input type="checkbox" name="status" class="form-checkbox" value="1" {{ old('status', $subtag ? $subtag->status : null) == '1' ? 'checked' : '' }} />
                                                <span class="mb-0">Enable this Subtag</span>
                                            </label>
                    ...
                    <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                        ...
                        <div class="flex items-start justify-center min-h-screen px-4"
                            ...
                            <div class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-4xl my-20 animate__animated"
                                ...
                                <div
                                    class="flex bg-gradient-to-b from-[#16222a] to-[#3a6073] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                                    <h5faq_view class="font-bold text-lg">View SubTags</h5faq_view>
                                    <button type="button" class="text-white-dark hover:text-dark"
                                        @click="toggle; window.location.reload();">
                                    ...
                                    <form  method="POST">
                                        <div>
                                            <div class="grid grid-cols-2 w-full gap-3">
                                                <div>
                                                    <label>SubTag Name - En</label>
                                                    <input disabled type="text" class="form-input" name="subTagNameEn" value="{{ $subtag_view ? $subtag_view->name : '' }}" placeholder="SubTag Name - En">
                                                    @error('subTagNameEn')
                                                        <small style="color: #b91c1c;" class="mt-1">{{ $message }}</small>
                                                    @enderror
                                                </div>
                                                <div>
                                                    <label>SubTag Name - Ar</label>
                                                    <input disabled type="text" class="form-input" name="subTagNameAr" value="{{ $subtag_view ? $subtag_view->name_ar : '' }}" placeholder="SubTag Name - Ar">
                                                    @error('subTagNameAr')
                                                        <small style="color: #b91c1c;" class="mt-1">{{ $message }}</small>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="w-full mt-3">
                                                <label>Sorting</label>
                                                <input disabled type="number" class="form-input" name="sorting" value="{{ $subtag_view ? $subtag_view->sorting : '' }}" placeholder="0">
                                            </div>
                                            <div class="w-full mt-3">
                                                <label>Upload Icon</label>
                                                <input disabled type="text" class="form-input" name="uploadIcon" value="{{ $subtag_view ? $subtag_view->icon : '' }}">
                                            </div>
                                            <div class="w-full mt-3">
                                                <label>Image Link App</label>
                                                <input disabled type="text" class="form-input" name="imageLink" value="{{ $subtag_view ? $subtag_view->image_link_app : '' }}">
                                            </div>
                                        </div>
                                        <div class="flex justify-between items-center mt-8">
                                            <label class="mb-0">
                                                <input disabled type="checkbox" name="status" class="form-checkbox" value="1" {{ old('status', $subtag_view ? $subtag_view->status : null) == '1' ? 'checked' : '' }} />
                                                <span class="mb-0">Enable this Subtag</span>
                                            </label>
    ...
    <script>
        const tagsData = [
            @foreach($subtags as $subtags)
                @php
                    $statusHtml = $subtags->status == 0
                        ? '<div class="text-danger font-semibold">Disabled</div>'
                        : '<div class="text-success font-semibold">Enabled</div>';
                @endphp
                [
                    `@if($permission && $permission->delete) <input name="ids[]" type="checkbox" value="{{ $subtags->id }}" class="delete-checkbox form-checkbox" /> @endif`,
                    `<span class='font-bold text-blue-600'>{{ $subtags->id }}</span>`,
                    `<span class=''>{{ $subtags->name }}</span>`,
                    `<span class=''>{{ $subtags->tag?->name }}</span>`,
                    `<span class=''>{{ $subtags->sorting }}</span>`,
                    `{!! $statusHtml !!}`,
                    `
                    <div class='flex gap-2'>
                        @if($permission && ($permission->view || $permission->view_all))
                            <a href="{{route('pim.tags.subtags.view',['id'=>$subtags->id])}}">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        @if($permission && $permission->edit)
                            <a href="{{route('pim.tags.subtags.edit',['id'=>$subtags->id])}}">
                        ...
                        @endif
                        @if($permission && $permission->delete)
                            <a href="{{route('pim.tags.subtags.delete',['id'=>$subtags->id])}}">
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: [
                                ...
                                "S No", "SubTag Name - En", "Family Name", "Sorting", "Status", "Action"
                            ],
                            data: this.tableData,
                        },
                        perPage: 10,
