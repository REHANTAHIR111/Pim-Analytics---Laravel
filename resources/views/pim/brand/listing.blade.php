        ...
        <div class="panel mt-6">
            <div class="flex gap-3 absolute">
                @if($permission && $permission->delete)
                    <form id="bulk-delete-form" method="POST" action="{{ route('pim.brand.bulk-delete') }}">
                        @csrf
                ...
                @if($permission && $permission->create)
                    <a href="{{ route('pim.brand.add') }}" type="button" class="btn btn-primary flex roles-top gap-1">
        ...
        const categoryData = [
            @foreach($brand as $brand)
                @php
                    ...
                    $statusHtml = $brand->status == 0
                        ? '<div class="text-danger font-semibold">Disabled</div>'
                        : '<div class="text-success font-semibold">Enabled</div>';
                    $mobileImage = $brand->mobile_image
                        ? "<img src='" . $brand->mobile_image . "' style='width:60px; height:45px;' />"
                        : "<img src='" . $defaultImage . "' style='width:60px; height:45px;' />";
                    $websiteImage = $brand->website_image
                        ? "<img src='" . $brand->website_image . "' style='width:60px; height:45px;' />"
                        : "<img src='" . $defaultImage . "' style='width:60px; height:45px;' />";

                @endphp
                [
                    `@if($permission && $permission->delete) <input name="ids[]" type="checkbox" value="{{ $brand->id }}" class="delete-checkbox form-checkbox" /> @endif`,
                    `<span class='font-bold text-info'>{{ $brand->id }}</span>`,
                    `<span class=''>{{ $brand->name }}</span>`,
                    `<span class=''>{{ $brand->sorting }}</span>`,
                    `{!! $websiteImage !!}`,
                    `{!! $mobileImage !!}`,
                    `{!! $statusHtml !!}`,
                    `
                    <div class='flex gap-2'>
                        @if($permission && ($permission->view || $permission->view_all))
                            <a href="{{route('pim.brand.view',['id'=>$brand->id])}}">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        @if($permission && $permission->edit)
                            <a href="{{route('pim.brand.edit',['id'=>$brand->id])}}">
                        ...
                        @endif
                        @if($permission && $permission->delete)
                            <a href="{{route('pim.brand.delete',['id'=>$brand->id])}}">
                        ...
                        data: {
                            headings: [
                                '@if($permission && $permission->delete) <input type="checkbox" class="form-checkbox" id="select-all" /> @endif',
                                 "S No", "Brand Name - En", "Sorting", "Images" , "App Images", "Status", "Actions",
                            ],
                            data: this.tableData,
                        },
                        perPage: 10,
