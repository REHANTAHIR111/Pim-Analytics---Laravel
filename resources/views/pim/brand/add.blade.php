<x-layout.default>
    <form action="{{ route('pim.brand.store') }}" method="POST" >
        @csrf
            ...
            <div class="px-0 flex-1 py-0 ltr:xl:mr-1 rtl:xl:ml-1">
                <div class="panel mb-5 flex items-center gap-3">
                    ...
                    <input
                        ...
                        value="http://127.0.0.1:8000/pim/brand/{{ basename(old('slug')) }}"
                        readonly
                    />
                ...
                <div class="mb-5" x-data="{
                    tab: 'English',
                    name: '{{ old('name') }}',
                    brand_description: '{{ old('brand_description') }}',
                    nameAr: '{{ old('nameAr') }}',
                    brand_descriptionAr: '{{ old('brand_descriptionAr') }}'
                }">
                    <div>
            ...
            <div class="xl:w-96 w-full xl:mt-0 mt-6">
                ...
                <div class="mb-3 panel space-y-3">
                    <div>
                        ...
                        <select name="selectCats[]" id="selectCats" class="w-full form-multiselect" multiple>
                            <option value="" disabled>Select</option>
                            @if(is_iterable($catOptions))
                                @foreach($catOptions as $cats)
                                    <option value="{{ $cats->id }}" {{ (collect(old('selectCats'))->contains($cats->id)) ? 'selected' : '' }}>
                                        {{ $cats->name }}
                                    </option>
                                @endforeach
                            @endif
                ...
                <div class="panel mb-3">
                    ...
                    <div class="relative rounded-md mt-3 overflow-hidden w-full max-w-sm" id="websitePreview">
                        @php $image_website = old('image_website') ?? $image_website ?? ''; @endphp
