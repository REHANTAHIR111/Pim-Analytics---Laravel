        ...
        <div class="panel mt-6">
            <div class="flex gap-3 absolute">
                @if($permission && $permission->delete)
                    <form id="bulk-delete-form" method="POST" action="{{ route('pim.product.bulk-delete') }}">
                        @csrf
                ...
                @if($permission && $permission->create)
                    <a href="{{ route('pim.product.add') }}" type="button" class="btn btn-primary flex gap-1">
        ...
        const productTableData = [
            @foreach($product as $product)
                @php
                    ...
                    $statusHtml = $product->status == 0
                        ? '<div class="text-danger font-semibold">Disabled</div>'
                        : '<div class="text-success font-semibold">Enabled</div>';
                    $preOrder = $product->enable_pre_order == 0
                        ? '<div class="text-danger font-semibold">Disabled</div>'
                        : '<div class="text-success font-semibold">Enabled</div>';
                    $mobileImage = $product->upload_featured_image
                        ? "<img src='" . $product->upload_featured_image . "' style='width:60px; height:45px;' />"
                        : "<img src='" . $defaultImage . "' style='width:60px; height:45px;' />";
                    $categories = ($product->categories && $product->categories->count() > 0)
                        ? $product->categories->pluck('name')->join(', ')
                        : '- - -';
                @endphp
                [
                    `@if($permission && $permission->delete) <input name="ids[]" type="checkbox" value="{{ $product->id }}" class="delete-checkbox form-checkbox" /> @endif`,
                    `<span class='font-bold text-info'>{{ $product->id }}</span>`,
                    `{!! $mobileImage !!}`,
                    `<span class=''>{{ $product->name }}</span>`,
                    `<span class=''>{{ $product->ln_sku }}</span>`,
                    `<span class=''>{{ $product->regular_price }}</span>`,
                    `<span class=''>{{ $product->sale_price }}</span>`,
                    `<div style='overflow-x: auto; max-width: 200px; white-space: nowrap;'>{{ $categories }}</div>`,
                    `<div style='overflow-x: auto; max-width: 200px; white-space: nowrap;'>{{ $product->notes }}</div>`,
                    `{!! $statusHtml !!}`,
                    `{!! $preOrder !!}`,
                    `
                    <div class='flex gap-2'>
                        @if($permission && ($permission->view || $permission->view_all))
                            <a href="{{route('pim.product.view',['id'=>$product->id])}}">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        @if($permission && $permission->edit)
                            <a href="{{route('pim.product.edit',['id'=>$product->id])}}">
                        ...
                        @endif
                        @if($permission && $permission->delete)
                            <a href="{{route('pim.product.delete',['id'=>$product->id])}}">
