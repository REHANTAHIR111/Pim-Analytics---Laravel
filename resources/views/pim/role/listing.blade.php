        ...
        <div class="panel mt-6">
            <div class="flex gap-3 absolute">
                @if($permission && $permission->delete)
                    <form id="bulk-delete-form" method="POST" action="{{ route('pim.role.bulk-delete') }}">
                        @csrf
                ...
                @if($permission && $permission->create)
                    <a href="{{ route('pim.role.add') }}" type="button" class="btn btn-primary flex gap-1">
    ...
    <script>
        const roleTableData = [
            @foreach($roles as $role)
                [
                    `@if($permission && $permission->delete) <input name="ids[]" type="checkbox" value="{{ $role->id }}" class="delete-checkbox form-checkbox" /> @endif`,
                    `<span class='font-bold text-info'>{{ $role->id }}</span>`,
                    `<span class='font-bold text-gray-600'>{{ $role->role }}</span>`,
                    `
                    <div class='flex gap-2'>
                        @if($permission && ($permission->view || $permission->view_all))
                            <a href="{{route('pim.role.view',['id'=>$role->id])}}">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        @if($permission && $permission->edit)
                            <a href="{{route('pim.role.edit',['id'=>$role->id])}}">
                        ...
                        @endif
                        @if($permission && $permission->delete)
                            <a href="{{route('pim.role.delete',['id'=>$role->id])}}">
