...
<header class="z-40" :class="{ 'dark': $store.app.semidark && $store.app.menu === 'horizontal' }">
    <div class="shadow-sm">
        <div class="relative flex w-full items-center px-5 py-2.5 dark:bg-[#0e1726] z-50 backdrop-blur-xl" style='background:linear-gradient(115deg, #16222a, #3a6073)'>
            <div class="horizontal-logo flex lg:hidden justify-between items-center ltr:mr-2 rtl:ml-2 gap-3">
                <a aria-current="page" class="main-logo flex items-center shrink-0 active" href="/">
                    <img class="h-9 w-auto ml-[5px] flex-none" src="/assets/images/logo.webp" alt="logo" style="filter: brightness(1) invert(1) hue-rotate(161deg);">
                </a>
                <a href="javascript:;"
            ...
            <h4 className='text-sm' style="
                color: #96dadc;
            ">
                Tamkeen Stores For Home & Appliances Backend Admin Panel
            </h4>
            <div x-data="header"
                ...
                <div class="dropdown flex-shrink-0" x-data="dropdown" @click.outside="open = false">
                    <a href="javascript:;" class="relative group" @click="toggle()">
                        <span><img class="w-9 h-9 rounded-full object-cover saturate-50 group-hover:saturate-100"
                                src="{{ $user && $user->profile_image ? asset($user->profile_image) : asset('/assets/images/user-profile.jpeg') }}" alt="image" /></span>
                    </a>
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms
                        ...
                        <li>
                            <div class="flex items-center px-4 py-4">
                                <div class="flex-none">
                                    <img class="rounded-md w-[3.5rem] h-[3.5rem] object-cover"
                                        src="{{ $user && $user->profile_image ? asset($user->profile_image) : asset('/assets/images/user-profile.jpeg') }}"
                                        alt="image" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4 truncate">
                                    <span class="text-xs font-bold bg-success-light rounded text-success px-1.5 py-0.5">{{ $user?->roleRelation?->role }}</span> <br>
                                    <h4 class="text-base line-clamp-1">{{ $user?->first_name }} {{ $user?->last_name }}</h4>
                        ...
                        <li class="border-t border-white-light dark:border-white-light/10">
                            <a href={{ route('auth.logout') }} class=" text-danger !py-3" @click="toggle">
