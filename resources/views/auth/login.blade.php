<x-layout.auth>
    <div>
        <div class="absolute inset-0" style='background:linear-gradient(90deg, #16222a, #3a6073)'>
            {{-- <img src="/assets/images/auth/bg-login.webp" alt="image" class="h-full w-full object-cover" /> --}}
        </div>
        <div class="relative flex min-h-screen items-center justify-center bg-[url(/assets/images/auth/map.png)] bg-cover bg-center bg-no-repeat px-6 py-10 dark:bg-[#060818] sm:px-16">
            ...
            <div
                class="relative w-full max-w-[670px] rounded-md bg-[linear-gradient(45deg,#fff9f9_0%,rgba(255,255,255,0)_25%,rgba(255,255,255,0)_75%,_#fff9f9_100%)] p-2 dark:bg-[linear-gradient(52.22deg,#0E1726_0%,rgba(14,23,38,0)_18.66%,rgba(14,23,38,0)_51.04%,rgba(14,23,38,0)_80.07%,#0E1726_100%)]">
                <div class="relative flex flex-col justify-center rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 px-4 min-h-[565px] py-10">
                    <div class="mx-auto w-full max-w-[400px]">
                        ...
                        <form class="space-y-5 dark:text-white" method="POST" action="{{ route('auth.login.submit') }}">
                            @csrf
                            <b class="text-red-500 text-sm mb-2">
                                @error('email')
                            ...
                            <div>
                                ...
                                @error('login')
                                <small style="color: #b91c1c;" class="mt-1">{{ $message !== 'Invalid credentials.' ? $message : '' }}</small>
                            @enderror
                            </div>
                            <div>
                                ...
                                <div class="relative text-gray-600">
                                    ...
                                    <button type="button" id="toggleCPassword" tabindex="-1"
                                        class="absolute end-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none">
                            ...
                            <div>
                                <label class="flex cursor-pointer items-center">
                                    <input type="checkbox" class="form-checkbox bg-white dark:bg-black" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }}/>
